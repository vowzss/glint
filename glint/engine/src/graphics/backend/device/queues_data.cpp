...
queues_data::queues_data(const VkDevice& device, const queue_families_support_details& families)
    : graphics(queue_data{device, families.graphics}), present(queue_data{device, families.present}), compute(queue_data{device, families.compute}),
      transfer(queue_data{device, families.transfer}), sparse(queue_data{device, families.sparseBinding}) {
}
...
//  ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy GLFW_KEY_* values are obsolete since 1.87 and not supported since 1.91.5] [X] Platform: Gamepad support. Enable
//  with 'io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad'. [X] Platform: Mouse cursor shape and visibility (ImGuiBackendFlags_HasMouseCursors) with
//  GLFW 3.1+. Resizing cursors requires GLFW 3.4+! Disable with 'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. [X] Multiple Dear ImGui contexts
//  support.
// Missing features or Issues:
//  [ ] Touch events are only correctly identified as Touch on Windows. This create issues with some interactions. GLFW doesn't provide a way to identify touch
//  inputs from mouse inputs, we cannot call io.AddMouseSourceEvent() to identify the source. We provide a Windows-specific workaround. [ ] Missing
...
// - Emscripten provides its own GLFW (3.2.1) implementation (syntax: "-sUSE_GLFW=3"), but Joystick is broken and several features are not supported (multiple
// windows, clipboard, timer, etc.)
// - A third-party Emscripten GLFW (3.4.0) implementation (syntax: "--use-port=contrib.glfw3") fixes the Joystick issue and implements all relevant features for
// the browser. See https://github.com/pongasoft/emscripten-glfw/blob/master/docs/Comparison.md for details.
...
//  2025-07-08: Made ImGui_ImplGlfw_GetContentScaleForWindow(), ImGui_ImplGlfw_GetContentScaleForMonitor() helpers return 1.0f on Emscripten and Android
//  platforms, matching macOS logic. (#8742, #8733) 2025-06-18: Added support for multiple Dear ImGui contexts. (#8676, #8239, #8069) 2025-06-11: Added
//  ImGui_ImplGlfw_GetContentScaleForWindow(GLFWwindow* window) and ImGui_ImplGlfw_GetContentScaleForMonitor(GLFWmonitor* monitor) helper to facilitate making
//  DPI-aware apps. 2025-03-10: Map GLFW_KEY_WORLD_1 and GLFW_KEY_WORLD_2 into ImGuiKey_Oem102. 2025-03-03: Fixed clipboard handler assertion when using GLFW
//  <= 3.2.1 compiled with asserts enabled. 2024-08-22: Moved some OS/backend related function pointers from ImGuiIO to ImGuiPlatformIO:
...
//  2024-07-08: *BREAKING* Renamed ImGui_ImplGlfw_InstallEmscriptenCanvasResizeCallback to ImGui_ImplGlfw_InstallEmscriptenCallbacks(), added GLFWWindow*
//  parameter. 2024-07-08: Emscripten: Added support for GLFW3 contrib port (GLFW 3.4.0 features + bug fixes): to enable, replace -sUSE_GLFW=3 with
//  --use-port=contrib.glfw3 (requires emscripten 3.1.59+) (https://github.com/pongasoft/emscripten-glfw) 2024-07-02: Emscripten: Added
//  io.PlatformOpenInShellFn() handler for Emscripten versions. 2023-12-19: Emscripten: Added ImGui_ImplGlfw_InstallEmscriptenCanvasResizeCallback() to register
//  canvas selector and auto-resize GLFW window. 2023-10-05: Inputs: Added support for extra ImGuiKey values: F13 to F24 function keys. 2023-07-18: Inputs:
//  Revert ignoring mouse data on GLFW_CURSOR_DISABLED as it can be used differently. User may set ImGuiConfigFLags_NoMouse if desired. (#5625, #6609)
...
//  handler to get more accurate scrolling impulses on Emscripten. (#4019, #6096) 2023-01-04: Inputs: Fixed mods state on Linux when using Alt-GR text input
...
//  GLFW_CURSOR_DISABLED by not setting mouse position *EDIT* Reverted 2023-07-18. 2022-04-30: Inputs: Fixed ImGui_ImplGlfw_TranslateUntranslatedKey() for lower
//  case letters on OSX. 2022-03-23: Inputs: Fixed a regression in 1.87 which resulted in keyboard modifiers events being reported incorrectly on Linux/X11.
...
void ImGui_ImplGlfw_MonitorCallback(GLFWmonitor*, int) {
    // Unused in 'master' branch but 'docking' branch will use this, so we declare it ahead of it so if you have to install callbacks you can install this one
    // too.
}
